...
- When a release is advertised as signed, open the file's Windows Digital
  Signatures properties and verify that the validated publisher matches the
  release documentation. An unsigned build must not claim a publisher.
...
- Review CSV logs, LTE history and ISP evidence ZIPs before sharing them.

## Release signing

Private keys, PFX files, certificate-provider credentials and signing passwords
must never be committed. The local release script selects a certificate already
installed in the Windows user certificate store by thumbprint, applies a trusted
timestamp and verifies Windows Authenticode policy. See
