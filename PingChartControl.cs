...
internal sealed class PingChartControl : Control
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        bool dark = BackColor.GetBrightness() < 0.35F;
        Color muted = dark ? Color.FromArgb(160, 170, 182) : Color.DimGray;
        using var titleBrush = new SolidBrush(ForeColor);
        using var mutedBrush = new SolidBrush(muted);
        using var axisPen = new Pen(dark
            ? Color.FromArgb(74, 82, 94)
            : Color.FromArgb(220, 225, 232));
        using var graphPen = new Pen(dark
            ? Color.FromArgb(74, 159, 235)
            : Color.FromArgb(35, 110, 195), 2);
        using var downPen = new Pen(Color.Firebrick, 2);

        e.Graphics.DrawString("Live ping history — last 180 samples",
            titleFont, titleBrush, 12, 10);

        if (Height < 100 || Width < 180)
        {
            e.Graphics.DrawString("Expand the window to show the graph.",
                smallFont, mutedBrush, 12, 35);
            return;
        }
        ...
        if (samples.Length < 2)
        {
            e.Graphics.DrawString("Waiting for measurements…",
                smallFont, mutedBrush, chart.Left + 10, chart.Top + 10);
            return;
        }
        ...
        e.Graphics.DrawString($"{max:0} ms",
            smallFont, mutedBrush, 5, chart.Top - 5);
        e.Graphics.DrawString("0 ms",
            smallFont, mutedBrush, 12, chart.Bottom - 10);
